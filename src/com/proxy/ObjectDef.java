...
public final class ObjectDef {
    ...
    public Model method578(int i, int j, int k, int l, int i1, int j1, int k1) {
        ...
        if (model == null)
            return null;
        if (aBoolean762 || shareLight)
            model = new Model(aBoolean762, shareLight, model);
        if (aBoolean762) {
            int l1 = (k + l + i1 + j1) / 4;
    ...
    public Model method581(int j, int k, int l) {
        ...
        if (anIntArray776 == null) {
            if (j != 10)
                return null;
            l1 = (long) (((long) type << 6) + l) + ((long) (k + 1) << 32);
        ...
        //!aBoolean769);
        model_3.method479(74, 1000, -90, -580, -90, !shareLight);
        if (anInt760 == 1)
    ...
    public void readValues474(Stream stream) {
        while (true) {
            ...
            decode474(stream, type);
        }
        //System.out.println("Decoded object: " + type + ", " + name);
        postDecode(flag == -1 && name != null && name != "null");
    }

    private void decode474(Stream stream, int type) {
        if (type == 1) {
            int modelCount = stream.readUnsignedByte();
            if (modelCount > 0) {
                if (anIntArray773 == null || lowMem) {
                    anIntArray776 = new int[modelCount];
                    anIntArray773 = new int[modelCount];
                    for (int modelIndex = 0; modelIndex < modelCount; modelIndex++) {
                        anIntArray773[modelIndex] = stream.readUnsignedWord();
                        anIntArray776[modelIndex] = stream.readUnsignedByte();
                    }
                } else {
...
//                anInt3806 = 1;
            ...
            int i1 = stream.readUnsignedByte();
            int[] modifiedModelColors = new int[i1];
            int[] originalModelColors = new int[i1];
            for (int i2 = 0; i2 < i1; i2++) {
                modifiedModelColors[i2] = stream.readUnsignedWord();
                originalModelColors[i2] = stream.readUnsignedWord();
            }
            ...
            childrenIDs[j1 + 1] = defaultId;
        } else if (type == 78) {
            int anInt3743 = stream.readUnsignedWord();
            int anInt3788 = stream.readUnsignedByte();
        } else if (type == 79) {
            int anInt3755 = stream.readUnsignedWord();
            int anInt3782 = stream.readUnsignedWord();
            int anInt3788 = stream.readUnsignedByte();
            int i_8_ = stream.readUnsignedByte();
            int[] anIntArray3801 = new int[i_8_];
            for (int i_9_ = 0; (i_9_ ^ 0xffffffff) > (i_8_ ^ 0xffffffff); i_9_++)
                anIntArray3801[i_9_] = stream.readUnsignedWord();
        } else if (type == 81) {
            int aByte3766 = (byte) 2;
            int aShort3789 = (short) (stream.readUnsignedByte() * 256);
        } else if (type == 82 || type == 88) {
        } else if (type == 89) {
            boolean aBoolean3768 = false;
        } else if (type == 90) {
            boolean aBoolean3741 = true;
        } else if (type == 91) {
            boolean aBoolean3759 = true;
        } else if (type == 93) {
            int aByte3766 = (byte) 3;
            short aShort3789 = (short) stream.readUnsignedWord();
        } else if (type == 94) {
            byte aByte3766 = (byte) 4;
        } else if (type == 95) {
            byte aByte3766 = (byte) 5;
        } else {
            throw new IllegalStateException("Error unrecognised config code: " + type);
        }
    }

    private void postDecode(boolean flag) {
        if (flag) {
