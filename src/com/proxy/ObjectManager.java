...
import static com.proxy.Model.isHdTexture;
...
final class ObjectManager {
    ...
    private final int[] underlay_chroma;
    private final int[] underlay_hue;
    private final int[] underlay_saturation;
    private final int[] underlay_luminance;
    private final int[] underlay_counts;
    ...
    public ObjectManager(byte abyte0[][][], int ai[][][]) {
        ...
        underlays = new byte[4][anInt146][anInt147];
        overlays = new byte[4][anInt146][anInt147];
        ...
        underlay_chroma = new int[anInt147];
        underlay_hue = new int[anInt147];
        underlay_saturation = new int[anInt147];
        underlay_luminance = new int[anInt147];
        underlay_counts = new int[anInt147];
    }
            ...
            for (int level = 0; level < 4; level++) {
                ...
                for (int j4 = 1; j4 < anInt147 - 1; j4++) {
                    for (int j5 = 1; j5 < anInt146 - 1; j5++) {
                        int k6 = anIntArrayArrayArray129[level][j5 + 1][j4] - anIntArrayArrayArray129[level][j5 - 1][j4];
                        int l7 = anIntArrayArrayArray129[level][j5][j4 + 1] - anIntArrayArrayArray129[level][j5][j4 - 1];
                ...
                for (int k5 = 0; k5 < anInt147; k5++) {
                    underlay_chroma[k5] = 0;
                    underlay_hue[k5] = 0;
                    underlay_saturation[k5] = 0;
                    underlay_luminance[k5] = 0;
                    underlay_counts[k5] = 0;
                }
                ...
                for (int x = -5; x < anInt146 + 5; x++) {
                    for (int i8 = 0; i8 < anInt147; i8++) {
                        int k9 = x + 5;
                        if (k9 >= 0 && k9 < anInt146) {
                            int l12 = underlays[level][k9][i8] & 0xff;
                            if (l12 > 0) {
                                Flo flo = Flo.underlay[l12 - 1];
                                underlay_chroma[i8] += flo.chroma;
                                underlay_hue[i8] += flo.hue;
                                underlay_saturation[i8] += flo.saturation;
                                underlay_luminance[i8] += flo.luminance;
                                underlay_counts[i8]++;
                            }
                        }
                        int i13 = x - 5;
                        if (i13 >= 0 && i13 < anInt146) {
                            int i14 = underlays[level][i13][i8] & 0xff;
                            if (i14 > 0) {
                                Flo flo_1 = Flo.underlay[i14 - 1];
                                underlay_chroma[i8] -= flo_1.chroma;
                                underlay_hue[i8] -= flo_1.hue;
                                underlay_saturation[i8] -= flo_1.saturation;
                                underlay_luminance[i8] -= flo_1.luminance;
                                underlay_counts[i8]--;
                            }
                        }
                    }
                    ...
                    if (x >= 1 && x < anInt146 - 1) {
                        ...
                        for (int y = -5; y < anInt147 + 5; y++) {
                            ...
                            if (j18 >= 0 && j18 < anInt147) {
                                chroma += underlay_chroma[j18];
                                hue += underlay_hue[j18];
                                saturation += underlay_saturation[j18];
                                luminance += underlay_luminance[j18];
                                count += underlay_counts[j18];
                            }
                            int k18 = y - 5;
                            if (k18 >= 0 && k18 < anInt147) {
                                chroma -= underlay_chroma[k18];
                                hue -= underlay_hue[k18];
                                saturation -= underlay_saturation[k18];
                                luminance -= underlay_luminance[k18];
                                count -= underlay_counts[k18];
                            }
                            if (y >= 1 && y < anInt147 - 1 && (!lowMem || (aByteArrayArrayArray149[0][x][y] & 2) != 0 || (aByteArrayArrayArray149[level][x][y] & 0x10) == 0 && method182(y, level, x) == anInt131)) {
                                if (level < anInt145) anInt145 = level;
                                int underlayId = underlays[level][x][y] & 0xff;
                                int overlayId = overlays[level][x][y] & 0xff;
                                if (underlayId > 0 || overlayId > 0) {
                                    int h1 = anIntArrayArrayArray129[level][x][y];
                                    int h2 = anIntArrayArrayArray129[level][x + 1][y];
                                    int h3 = anIntArrayArrayArray129[level][x + 1][y + 1];
                                    int h4 = anIntArrayArrayArray129[level][x][y + 1];
                                    ...
                                    int underlayColour = -1;
                                    int k21 = -1;
                                    if (underlayId > 0) {
                                        underlayColour = pack_hsl(hue / count, 256 * chroma / luminance, saturation / count);
                                        k21 = underlayColour;
                                    }
                                    if (level > 0) {
                                        boolean flag = true;
                                        if (underlayId == 0 && aByteArrayArrayArray136[level][x][y] != 0) flag = false;
                                        if (overlayId > 0 && !Flo.overlay[overlayId - 1].aBoolean393) flag = false;
                                        if (flag && h1 == h2 && h1 == h3 && h1 == h4)
                                            anIntArrayArrayArray135[level][x][y] |= 0x924;
                                    }
                                    int underlayMinimapRgb = 0;
                                    if (underlayColour != -1)
                                        underlayMinimapRgb = Rasterizer.HSLtoRGB[method187(k21, 96)];

                                    if (overlayId == 0) {
                                        worldController.method279(level, x, y, 0, 0, -1, h1, h2, h3, h4, method187(underlayColour, j20), method187(underlayColour, k20), method187(underlayColour, l20), method187(underlayColour, i21), 0, 0, 0, 0, underlayMinimapRgb, 0);
                                    } else {
                                        int k22 = aByteArrayArrayArray136[level][x][y] + 1;
                                        byte byte4 = aByteArrayArrayArray148[level][x][y];
                                        Flo flo = Flo.overlay[overlayId - 1];
                                        int textureId = flo.textureId;
                                        if (isHdTexture(textureId)) {
                                            textureId = -1;
                                        }
                                        int overlayColour;
                                        int overlayMinimapRgb;
                                        if (textureId >= 0) {
                                            overlayColour = -1;
                                            overlayMinimapRgb = Rasterizer.method369(textureId);
                                        } else if (flo.tile_colour == -1) {
                                            overlayColour = -2;
                                            overlayMinimapRgb = 0;
                                        } else {
                                            overlayColour = flo.tile_colour;
                                            overlayMinimapRgb = Rasterizer.HSLtoRGB[method185(flo.tile_colour, 96)];
                                        }
                                        worldController.method279(level, x, y, k22, byte4, textureId, h1, h2, h3, h4, method187(underlayColour, j20), method187(underlayColour, k20), method187(underlayColour, l20), method187(underlayColour, i21), method185(overlayColour, j20), method185(overlayColour, k20), method185(overlayColour, l20), method185(overlayColour, i21), underlayMinimapRgb, overlayMinimapRgb);
                                    }
                                }
                            }
                        }
                ...
                for (int j8 = 1; j8 < anInt147 - 1; j8++) {
                    for (int i10 = 1; i10 < anInt146 - 1; i10++)
                        worldController.method278(level, i10, j8, method182(j8, level, i10));
    ...
    private int pack_hsl(int var2, int var3, int var0) {
        if (-244 > ~var0) {
            var2 >>= 4;
        } else {
            if (~var0 < -218) {
        ...
        for (int j1 = i; j1 <= i + j; j1++) {
            for (int k1 = i1; k1 <= i1 + l; k1++)
                if (k1 >= 0 && k1 < anInt146 && j1 >= 0 && j1 < anInt147) {
                    ...
                    if (k1 == i1 && k1 > 0) anIntArrayArrayArray129[0][k1][j1] = anIntArrayArrayArray129[0][k1 - 1][j1];
                    if (k1 == i1 + l && k1 < anInt146 - 1)
                        anIntArrayArrayArray129[0][k1][j1] = anIntArrayArrayArray129[0][k1 + 1][j1];
                    if (j1 == i && j1 > 0) anIntArrayArrayArray129[0][k1][j1] = anIntArrayArrayArray129[0][k1][j1 - 1];
    ...
    private void method175(int i, WorldController worldController, Class11 class11, int j, int k, int l, int i1, int j1) {
        ...
        if (j == 0) {
            ...
            if (class46.aBoolean767 && class11 != null) class11.method211(i, j1, l, j, class46.aBoolean757);
        ...
        if (j == 1) {
            ...
            if (class46.aBoolean779) if (j1 == 0) aByteArrayArrayArray134[k][l][i + 1] = 50;
        ...
        if (j == 2) {
            ...
            if (class46.aBoolean764) if (j1 == 0) {
        ...
        if (class46.aBoolean762) if (j1 == 1) {
            int j3 = j2;
            j2 = i2;
            i2 = l1;
            l1 = k1;
        ...
        try {
            if (k >= 0 && k < 104 && i >= 0 && i < 104) {
                ...
                do {
                    ...
                    else underlays[l][k][i] = (byte) (l1 - 81);
                } while (true);
            }
            do {
    ...
    private int method182(int i, int j, int k) {
        if ((aByteArrayArrayArray149[j][k][i] & 8) != 0) return 0;
        if (j > 0 && (aByteArrayArrayArray149[1][k][i] & 2) != 0) return j - 1;
        else return j;
    }
